...
def ValidateNumber(random_number,User_Selected_Number):
    global no_of_chances, Remaining_chances, Bull_count, cow_count 
    #no_of_chances += 1
    #Remaining_chances = 7
    ...
    while(no_of_chances<=8):
        ...
        if Bull_count !=0 and cow_count !=0 and Remaining_chances!=0:
            ...
            print("Oops, You are unable to guess the number in given 8 chances and the number is {}:".format(random_number))
            selected_option = int(input("1.Exit\t 2.Play Again"))
            if selected_option == 1:
                exit
            else:
                getdigitValues(random_number,User_Selected_Number)
                ValidateNumber(random_number,User_Selected_Number)                     
        else:
...
User_Selected_Number = int(input("Hi {}, We are already generated a number try to guess that number[Number should be 3-digit and non-negative - 100 to 999] :".format(User_Name)))
